...
function CheckoutForm({ roundId, amount, regionName, onClose }) {
...
// Props: round (object with .id, .region_name), open (bool), onClose (fn)
export default function DonationModal({ round, open, onClose }) {
      ...
      <div className="relative w-full max-w-lg bg-[#080f1a] rounded-[32px] border border-white/10 shadow-2xl overflow-hidden" style={{ animation: "modalSlideUp 0.22s cubic-bezier(0.16,1,0.3,1)" }}>
        ...
        <div className="p-8">
          ...
          <div className="flex items-start justify-between mb-8">
            <div>
              ...
              <h2 className="text-2xl font-bold text-white">
                Intervene in {round.region_name ?? "Region"}
              </h2>
            </div>
          ...
          <Elements stripe={stripePromise}>
            <CheckoutForm roundId={round.id} amount={amount} regionName={round.region_name} onClose={onClose} />
          </Elements>
        </div>
      </div>
    </div>
